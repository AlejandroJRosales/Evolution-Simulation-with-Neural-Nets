		...
		if generation % print_every == 0:
			...
			for i in range(len(final[0])):
				...
				if i == 0:
					...
					print("Score", " " * ((max_len - current_len) + 2), end=" ")
				else:
...
def new_blood(weights, humans_medians, gritiss_medians, drakonians_medians, human_count, gritis_count, drakonian_count):
	...
	speed_mutator = .25
	power_mutator = .25
	if human_count != 0 and dom_species != species[0]:
		human = [('Species', species[0])]
	...
	elif gritis_count != 0 and dom_species != species[1]:
		gritis = [('Species', species[1])]
	...
	elif drakonian_count != 0 and dom_species != species[2]:
		drakonian = [('Species', species[2])]
